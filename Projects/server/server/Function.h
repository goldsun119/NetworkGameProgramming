...
void SetInitData(PlayerInfo a, int num);	// 초기 데이터 전송
...
DWORD WINAPI ProcessClient(LPVOID arg) {
	...
	retval = send(ClientSock, (char*)&playerInfo[ClientNum], sizeof(playerInfo[ClientNum]), 0);//초기 데이터 전송
	if (retval == SOCKET_ERROR) {
		err_display("send() InitData");
	}
	while (true)
	{
